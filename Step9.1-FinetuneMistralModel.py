...
validation_samples = 500  # Number of validation samples to use
test_samples = 100     # Number of test samples to use

train_samples = 100  # Number of training samples to use
validation_samples = 10  # Number of validation samples to use
...
dtype = None
load_in_4bit = True
# Use Mistral v0.3 model
model_name = "unsloth/mistral-7b-instruct-v0.3-bnb-4bit"  # Using 4-bit quantized version for faster loading
num_epochs = 50        # Number of training epochs
output_dir = "outputs_mistral" # Directory to save outputs
...
# Set up chat template for Mistral
...
# ====== DATASET PREPARATION ======
# Convert prompt/completion format to the chatml format for Mistral
def create_conversation_data(data):
    formatted_data = []
    for item in data:
        # Create a conversation with user prompt and assistant completion
        conversation = [
...
# Function to format using Mistral's ChatML template
def formatting_prompts_func(examples):
    convos = examples["conversations"]
    texts = [tokenizer.apply_chat_template(convo, tokenize = False, add_generation_prompt = False) for convo in convos]
    return { "text" : texts, }

# Convert raw data to conversations format
train_conversations = create_conversation_data(train_data)
val_conversations = create_conversation_data(val_data)
...
def evaluate_model(model, tokenizer, test_data, epoch):
    ...
    for i, item in enumerate(test_data):
        ...
        try:
            # For ChatML format used by Mistral
            assistant_start = full_output.find("<|im_start|>assistant")
            if assistant_start != -1:
                # Get content after "<|im_start|>assistant"
                content_start = full_output.find("\n", assistant_start)
                if content_start != -1:
                    prediction = full_output[content_start:].strip()
                    # Remove any trailing markers
                    if "<|im_end|>" in prediction:
                        prediction = prediction.split("<|im_end|>")[0].strip()
            else:
                # Fallback to taking everything after the user's message
...
# ====== TRAINING SETUP ======
# Using improved training arguments for the Mistral model
training_args = TrainingArguments(
   ...
   learning_rate=2e-4,               # Learning rate for Mistral
...
early_stopping = EarlyStoppingCallback(patience=500, min_delta=0.01)
gradient_monitor = GradientMonitoringCallback(model, log_freq=50)

# Setup trainer for Mistral
trainer = SFTTrainer(
   model=model,
   tokenizer=tokenizer,
...
# ====== SAVE FINAL MODEL ======
print("Saving final model to 'mistral_v0_3_finetuned'")
model.save_pretrained("mistral_v0_3_finetuned")
tokenizer.save_pretrained("mistral_v0_3_finetuned")
...
model.save_pretrained_merged("mistral_v0_3_deployment", tokenizer, save_method="merged_16bit")

# Save in 4bit for deployment
model.save_pretrained_merged("mistral_v0_3_deployment_4bit", tokenizer, save_method="merged_4bit")
...
print(f"Final model saved to 'mistral_v0_3_finetuned'")
