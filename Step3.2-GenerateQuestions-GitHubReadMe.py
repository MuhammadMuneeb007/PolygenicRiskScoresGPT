...
GOOGLE_API_KEY = "AIzaSyBawXYh6-f-_QU_hOp_UqfyxOt3mkCU_No"
...
def generate_questions(tool_name, text_chunk, link=None, chunk_id=None):
    ...
    prompt = f"""
    You are an expert in generating high-quality training data for language models. Your task is to create **exactly 100** question-answer pairs about **{tool_name}**, using the GitHub readme file for the tool.  
    ...
    - Ensure all questions are **fully supported** by the provided GithUB readme.
    - Avoid referencing specific locations in the GithUB (e.g., "as mentioned in Equation 2" or "refer to Section 4").
    ...
    - Since the manual is part of a **Github Readme**, prioritize:
    - **execution**
    - **Technical depth**
    - **instalation. rigor**
...
def process_github():
    ...
    for tool_name in tool_dirs:
        ...
        article_dir = os.path.join(tools_path, tool_name, "GitHubReadme")
        article_path = os.path.join(article_dir,   "README.md")
        ...
        article_link = ""
        if not tool_row.empty and 'GitHubReadme' in tool_row.columns:
            article_link = str(tool_row["GitHubReadme"].values[0]).strip() if not pd.isna(tool_row["GitHubReadme"].values[0]) else ""
            
        # Generate questions from the article
        article_questions = generate_questions(tool_name, article_text, link=article_link   )
