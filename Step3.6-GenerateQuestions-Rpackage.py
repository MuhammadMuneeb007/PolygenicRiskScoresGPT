...
GOOGLE_API_KEY = "AIzaSyC21OlY6GEoQ9oENx6NibyE_VIt_A5Hdwc"
...
def process_github():
    ...
    for tool_name in tool_dirs:
        ...
        article_dir = os.path.join(tools_path, tool_name, "Rpackage")
        ...
        # MD files and RMD files
        mdfiles = ""
        rmdfiles = ""
        
        md_path = os.path.join(article_dir, "MD.txt")
        rmd_path = os.path.join(article_dir, "RMD.txt")
        ...
        if not mdfiles and not rmdfiles:
            print(f"No MD.txt or RMD.txt files found in {article_dir}")
            continue
        
        # Merge the information from MD and RMD files
        article_text = mdfiles + rmdfiles
        
        # Get article link from Excel if available
        tool_row = tools_df[tools_df['Tool'] == tool_name]
        article_link = ""
        if not tool_row.empty and 'Rpackage' in tool_row.columns:
            article_link = str(tool_row["Rpackage"].values[0]).strip() if not pd.isna(tool_row["Rpackage"].values[0]) else ""
            
        # Check if the text is large and needs to be split into chunks
        ...
        if word_count > 8000:
            ...
            for i, chunk in enumerate(chunks):
                ...
                chunk_questions = generate_questions(tool_name, chunk, link=article_link, chunk_id=i+1)
            ...
            all_questions = generate_questions(tool_name, article_text, link=article_link)
